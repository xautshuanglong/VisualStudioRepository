...
void LogTool::BasicConfigration()
{
	...
	// 输出日志到控制台
	pTempFileLayout->setConversionPattern(log4cxx::LogString(TEXT("%d{HH:mm:ss.SSS} [%-5p] %C.%M %m (%F:%L)%n")));
	...
	// 输出日志到回滚文件（用于长期记录）
	pRollingFileLayout->setConversionPattern(log4cxx::LogString(TEXT("%d{yyyy-MM-dd HH:mm:ss.SSS} [%-5p] %C.%M %m (%F:%L)%n")));
